...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!pocketSecret) {
    return Response.json(
      { ok: false, error: "Missing POCKET_WEBHOOK_SECRET configuration." },
      { status: 500 }
    );
  }
  ...
  try {
    ...
    const structuredMarkdown = await generateStructuredMarkdown({
      title,
      summaryMarkdown,
      transcript,
    });
    ...
    console.error("Pocket webhook processing failed:", error);
